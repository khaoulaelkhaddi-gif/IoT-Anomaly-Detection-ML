...
rf_model = RandomForestClassifier(n_estimators=100, random_state=50, n_jobs=-1)
...
feature_importance_df['Importance_%'] = feature_importance_df['Importance'] * 100.0
...
for i, imp_pct in enumerate(ranked_top['Importance_%']):
    ax.text(imp_pct + 0.1, i, f"{imp_pct:.2f}%", va='center')
plt.title('Random Forest Feature Importance Ranking - TON-IoT dataset')
plt.xlabel('Importance Weight (%)')
plt.ylabel('Ranked Features')
plt.tight_layout()
plt.savefig('chart_rf_feature_importance_ranking_Public.png')
plt.close()

#  Keep ONLY the Top-10 features
k = min(10, len(feature_importance_df))
top_features = feature_importance_df['Feature'].head(k).tolist()
...
train_cleaned.to_csv('train_cleaned_rf_public.csv', index=False)
test_cleaned.to_csv('test_cleaned_rf_public.csv', index=False)
